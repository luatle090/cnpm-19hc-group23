...
module.exports = {
  ...
  checkUnique: (userId, soTK) => {
    const sql = `select id from thietlapnguoinhan 
                where idTaiKhoanNHGui = ${userId} and soTaiKhoanNhan = ${soTK}`;
    return db.load(sql);
  },

  getInfoThiepLapByUserId: id => {
    const sql = `SELECT tk.soTK, kh.hoTen, tl.tenGoiNho
